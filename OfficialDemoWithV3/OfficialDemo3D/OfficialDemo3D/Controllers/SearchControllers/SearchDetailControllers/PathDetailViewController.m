...
- (NSString *)subTitleForIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0)
    {
        switch (indexPath.row)
        {
            case 0: subTitle = [NSString stringWithFormat:@"%d(米)", self.path.distance];    break;
            case 1: subTitle = [NSString stringWithFormat:@"%d(秒)", self.path.duration];    break;
            case 2: subTitle = self.path.strategy;                                           break;
            case 3: subTitle = [NSString stringWithFormat:@"%f(元)", self.path.tolls];        break;
            default:subTitle = [NSString stringWithFormat:@"%d(米)", self.path.tollDistance]; break;
        }
    }
    else
    {
