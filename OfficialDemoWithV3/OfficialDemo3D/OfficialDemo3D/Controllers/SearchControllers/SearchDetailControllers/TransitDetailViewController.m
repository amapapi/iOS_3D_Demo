...
- (NSString *)subTitleForIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0)
    {
        switch (indexPath.row)
        {
            ...
            case 1: subTitle = [NSString stringWithFormat:@"%d(秒)", self.transit.duration]; break;
            case 2: subTitle = [NSString stringWithFormat:@"%@", self.transit.nightflag ? @"YES" : @"NO"]; break;
            default:subTitle = [NSString stringWithFormat:@"%d(米)", self.transit.walkingDistance]; break;
        }
    }
    else
    {
