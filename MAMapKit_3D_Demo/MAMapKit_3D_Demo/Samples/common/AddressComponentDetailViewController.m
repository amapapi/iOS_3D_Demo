...
- (void)initTableData
{
    ...
    if (self.addressComponent.businessAreas.count > 0)
    {
        ...
        businessAreas = [ba name];
    }
    self.subTitleArray = @[self.addressComponent.province,
                           [NSString stringWithFormat:@"%@(%@)", [self emptyStringIfNil:self.addressComponent.city], [self emptyStringIfNil:self.addressComponent.citycode]],
                           [NSString stringWithFormat:@"%@(%@)", [self emptyStringIfNil:self.addressComponent.district], [self emptyStringIfNil:self.addressComponent.adcode]],
                           [NSString stringWithFormat:@"%@(%@)", [self emptyStringIfNil:self.addressComponent.township], [self emptyStringIfNil:self.addressComponent.towncode]],
                           [self emptyStringIfNil:self.addressComponent.neighborhood],
                           [self emptyStringIfNil:self.addressComponent.building],
                           [NSString stringWithFormat:@"%@%@",self.addressComponent.streetNumber.street,self.addressComponent.streetNumber.number],
                           businessAreas];
    
    self.titleArray = @[@"省", @"市(cityCode)", @"地区(adCode)", @"乡镇(townCode)", @"社区", @"建筑", @"门牌信息", @"商圈"];
}
